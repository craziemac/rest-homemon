...
function handleGet(connection,req,res) {
    ...
    connection.query('SELECT * FROM SENSOR ORDER BY SENS_Ruid ASC LIMIT ' + limit, function handleSql(err, rows) {
        if (err){ logAndRespond(err,res); return; }
...
function handleFind(connection,req,res) {
    var find = function find(id){
        connection.query('SELECT * FROM SENSOR WHERE SENS_Ruid = ?', id, function handleSql(err, rows) {
            if (err){ logAndRespond(err,res); return; }
            if (rows.length === 0){ res.send(204); return; }
            res.send({
                result: 'success',
...
function handleIns(connection,req,res) {
    connection.query('INSERT INTO SENSOR SET ?', req.body, function handleSql(err, result) {
        if (err){ logAndRespond(err,res); return; }
        res.statusCode = 201;
        res.send({
            result: 'success',
...
function handleUpd(connection,req,res) {
    connection.query('UPDATE SENSOR SET ? WHERE SENS_Ruid='+req.params.id, req.query, function handleSql(err) {
        if (err){ logAndRespond(err,res); return; }
        handleFind(connection,req,res)
    });
}
function handleDel(connection,req,res) {
    connection.query('DELETE FROM SENSOR WHERE SENS_Ruid = ?', req.params.id, function handleSql(err) {
        if (err){ logAndRespond(err,res); return; }
        res.send({
            result: 'success',
            err:    '',
